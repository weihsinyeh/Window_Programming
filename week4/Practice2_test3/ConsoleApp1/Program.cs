...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static int[] next_row = { 0, -1, 1, 0, 0 };
        static int[] next_col = { 1, 0, 0, -1, 0 };
        struct position
        {
            public int row, col;
        }
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                int[,] fork_count_array = new int[row,col];
                for (int i = 0; i < row; i++)
                {
        ...
        static void fork_count(char[,] array, int [,] fork_count_array)
        {
            fork_count_array = new int[array.GetLength(0), array.GetLength(1)];
            for (int k = 1; k < array.GetLength(0) - 1; k++)
            {
                for (int m = 1; m < array.GetLength(1) - 1; m++)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        //深度優先 fork_count_array[k, m]==3 為交叉口
                        if ((array[k + next_row[i], m + next_col[i]] == ' ') || (array[k + next_row[i], m + next_col[i]] == '0'))
                        {
                            fork_count_array[k, m]++;
                        }
                        if (array[k, m] == '1')
                        {
                            fork_count_array[k, m] = 1;
                        }
                        if (array[k, m] == 'X')
                        {
                            fork_count_array[k, m] = -1;
